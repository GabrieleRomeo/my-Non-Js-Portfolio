...
gulp.task('update-version', HELPS.updateVersion, () => {
...
gulp.task('server', 'Start the browserSync server', () => {
...
gulp.task('deploy', HELPS.deploy, callback => {
    ...
    let currentBranch = shell.exec('git rev-parse --abbrev-ref HEAD', {silent:true}).stdout;

    // Check if the deploy task has been started from the deploy branch
    if (currentBranch.indexOf(RELEASEBRANCH) === -1) {
        shell.echo(gutil.colors.yellow(`Start the deploy task from the
    ...
    fs.access(path.join(PATHS.DIST_DIR, 'index.html'), fs.F_OK, err => {
        if (err) {
            gutil.log(gutil.colors.red(`ERROR: The index.html file does not
